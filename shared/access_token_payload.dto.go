...
type AccessTokenPayloadDTO struct {
	ID          uint     `bind:"id"`
...
func (self AccessTokenPayloadDTO) Transform(c gooh.Context, medata common.ArgumentMetadata) any {
	tokenClaims := c.Context().Value("tokenClaims").(jwt.MapClaims)
	accessTokenPayloadDTO, _ := ctx.BindStruct(tokenClaims, &[]ctx.FieldLevel{}, AccessTokenPayloadDTO{}, "")
